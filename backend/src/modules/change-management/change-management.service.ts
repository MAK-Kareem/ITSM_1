...
export class ChangeManagementService {
  ...
  async update(
    ...
    // Check if Line Manager is being changed
    let lmChanged = false;
    if (updateDto.lineManagerId && updateDto.lineManagerId !== cr.lineManagerId) {
      cr.lineManagerId = updateDto.lineManagerId;
      cr.lineManager = null; // Force reload relation
      lmChanged = true;
    }
    ...
    // If Line Manager changed, send notification to new Line Manager
    if (lmChanged) {
      const fullCr = await this.findOne(id); // Reload to get new LM details
      await this.sendNotification(fullCr, 'CR_CREATED', [fullCr.lineManagerId]); 
    }
   ...
   * FIXED: Line Manager visibility, IT Officer visibility issues
  ...
  async findByRole(userId: number, userRole: string, viewAll: boolean = false): Promise<ChangeRequest[]> {
    ...
    switch (userRole) {
      ...
      case 'line_manager':
        // FIXED: Line Manager sees CRs at Stage 2 (Pending LM Approval) assigned to them
        query.andWhere('cr.lineManagerId = :userId', { userId })
             .andWhere('cr.currentStage >= :minStage', { minStage: 2 });
        break;

      case 'head_of_it':
        // Head of IT sees CRs that have passed LM approval (Stage 3+)
        query.andWhere('cr.currentStage >= :stage', { stage: 3 });
        break;

      case 'it_officer':
        // FIXED: IT Officer sees CRs assigned to them at Stage 4+ (including after deployment)
        query.andWhere('cr.assignedToITOfficerId = :userId', { userId })
             .andWhere('cr.currentStage >= :minStage', { minStage: 4 });
        break;

      case 'qa_officer':
        // QA sees CRs at Stage 6 (Pending QA Validation)
        query.andWhere('cr.currentStage = :stage', { stage: 6 });
        break;

      case 'head_of_infosec':
        // Head of InfoSec sees CRs at Stage 8 (Pending InfoSec approval)
        query.andWhere('cr.currentStage = :stage', { stage: 8 });
        break;

      case 'noc':
        // FIXED: NOC sees CRs at Stage 10 (Waiting for closure) AND completed ones
        query.andWhere('(cr.currentStage = :stage OR cr.currentStatus = :completed)', { 
          stage: 10,
          completed: 'Completed'
        });
        break;
  ...
  async approve(
    ...
    // Explicitly associate 'changeRequest' to avoid TypeORM ignoring crId
    const approval = this.approvalRepository.create({
      crId: id,
      changeRequest: cr, // <-- Added this
  ...
  async reject(
    ...
    // Explicitly associate 'changeRequest' here as well
    const approval = this.approvalRepository.create({
      crId: id,
      changeRequest: cr, // <-- Added this
      stage: cr.currentStage,
  ...
  async addDeploymentTeamMember(
    ...
    // FIXED: Requirement 11 - Send email notification to deployment team member
    const cr = await this.findOne(id);
    await this.sendNotification(cr, 'DEPLOYMENT_TEAM_ADDED', []);
  ...
  async getCRHistory(id: number): Promise<CRHistory[]> {
    // FIXED: Requirement 19 - Order history from bottom to top (Stage 1 at top)
  ...
  async closeCR(
    ...
    // Explicitly associate 'changeRequest' here too
    const approval = this.approvalRepository.create({
      crId: id,
      changeRequest: cr, // <-- Added this
      stage: 10,
  ...
  private async sendNotification(
    ...
    try {
      ...
      if (validRecipients.length > 0 || notificationType === 'DEPLOYMENT_TEAM_ADDED') {
   ...
   * FIXED: Requirements 6, 10, 11 - IT Officer, HoIT, and deployment team notifications
   */
  private async sendStageNotifications(
    ...
    const notificationMap: Record<number, { type: string; recipients: number[] }> = {
      // Stage 3: After LM approval → Notify HoIT (FIXED: Requirement 10)
      3: {
        type: 'LM_APPROVED',
        recipients: [], // HoIT users - will be looked up by role in email service
      },

      // Stage 4: After HoIT approval → Notify assigned IT Officer (FIXED: Requirement 6)
      4: {
        type: 'HOIT_APPROVED',
