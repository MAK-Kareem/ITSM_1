...
const ChangeRequestDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    if (id && activeTab === 'approvals') {
      forceRefreshApprovals();
    }
  }, [activeTab, id]);

  const forceRefreshApprovals = async () => {
    try {
      const data = await changeRequestService.getApprovals(Number(id));
  ...
  const loadCRData = async () => {
    ...
    try {
      ...
      if (crData.approvals && crData.approvals.length > 0) {
        setApprovals(crData.approvals);
      } else {
        // Fallback: try explicit fetch if embedded data is empty
        forceRefreshApprovals();
      }

      if (crData.history) setHistory(crData.history);
      if (crData.deploymentTeam) setDeploymentTeam(crData.deploymentTeam);
      if (crData.testingResults) setTestingResults(crData.testingResults);
      if (crData.qaChecklists) setQAChecklists(crData.qaChecklists);
      ...
      <Card title="Change Request Information" style={{ marginBottom: 16 }}>
        <Descriptions bordered column={{ xs: 1, sm: 2, md: 3 }}>
          ...
          <Descriptions.Item label="Status" span={1}>
            <Tag style={getStatusStyle(cr.currentStatus)}>
              {cr.currentStatus}
            </Tag>
          </Descriptions.Item>
          ...
          items={CR_STAGES.map((stage) => ({
            ...
            description: (
              <div style={{ fontSize: '12px', color: '#8c8c8c' }}>
                <div style={{ marginBottom: 4 }}>{stage.description}</div>
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {canUserActOnCurrentStage() && cr.currentStatus !== 'Rejected' && cr.currentStatus !== 'Completed' && (
        <Alert
          message={<span style={{ fontWeight: 'bold', color: '#fff', fontSize: '15px' }}>Action Required</span>}
          description={<span style={{ color: 'rgba(255,255,255,0.9)' }}>{`You have pending action on this CR at Stage ${cr.currentStage}: ${getCurrentStageName(cr.currentStage)}`}</span>}
          type="warning"
          showIcon
          icon={<ExclamationCircleOutlined style={{ color: '#faad14', fontSize: '18px' }} />}
          style={{ 
            marginBottom: 16,
      ...
      <Tabs activeKey={activeTab} onChange={setActiveTab}>
        ...
        >
          <Row gutter={24}>
            <Col xs={24} lg={20}> {/* Increased Main Content Width to ~83% */}
              {renderBasicInfo()}
              {renderITOfficerInfo()}
              ...
              {canUserActOnCurrentStage() &&
                ...
                cr.currentStatus !== 'Completed' && (
                  <Card
                    ...
                    style={{ marginBottom: 16 }}
                    headStyle={{ 
                      backgroundColor: '#fffbe6', // Lightened background color
                      borderBottom: '1px solid #ffe58f', // Lightened border
                      padding: '12px 16px',
                      fontWeight: 600
                    }}
                  >
            ...
            <Col xs={24} lg={4}> {/* Reduced Workflow Progress Width to ~17% */}
