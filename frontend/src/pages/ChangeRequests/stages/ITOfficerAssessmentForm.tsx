...
interface TestResult {
  key: string;
  testCase: string;
  testResult: string;  // Merged field
  passed: boolean;
  remarks: string;
}
...
const ITOfficerAssessmentForm: React.FC<Props> = ({ cr, onSuccess }) => {
  ...
  const handleSubmit = async (values: any) => {
    ...
    const invalidTest = testResults.find(
      (t) => !t.testCase.trim() || !t.testResult.trim()
    );
    if (invalidTest) {
      message.error('All test results must have test case and result description');
      return;
    }
    ...
    try {
      ...
      const testingData = {
        ...
        testResults: testResults.map((t) => ({
          testCase: t.testCase,
          expectedResult: t.testResult,  // Use single field for both
          actualResult: t.testResult,     // Use single field for both
          passed: t.passed,
          remarks: t.remarks,
        })),
              ...
              <Row gutter={16}>
                ...
                <Col span={24}>
                  <TextArea
                    ...
                    onChange={(e) => updateTestResult(test.key, 'testResult', e.target.value)}
                    placeholder="Test Result (what happened during testing)"
                    rows={3}
                    style={{ marginBottom: 8 }}
                  />
                </Col>
